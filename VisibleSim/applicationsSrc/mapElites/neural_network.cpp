...
/**
 * @brief Check if the weights are of the right size
 * 
*/
void NeuralNetwork::checkWeights() {
    // ! error handling, check if weights are of the right size
    if (weights.size() != static_cast<size_t>(numHiddenLayers + 1)) {
        throw runtime_error("\033[1;31m[WEIGHTS]\033[0m Invalid number of weight layers, expected " + to_string(numHiddenLayers + 1) + " got " + to_string(weights.size()));
    }
...
/**
 * @brief Generate random weights for the network
 * 
 * @return vector<vector<vector<double>>> the random weights
*/
vector<vector<vector<double>>> NeuralNetwork::generateRandomWeights() {
    ...
    for (int i = 0; i < this->numHiddenLayers + 1; i++) {
        ...
        for (auto &innerVec : this->weights[i]) {
            for (double &weight : innerVec) {
                weight = static_cast<double>(rand()) / RAND_MAX; // generate a random weight between 0 and 1
            }
        }
    }
